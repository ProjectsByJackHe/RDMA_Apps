...
public:
        ...
        _In_ DWORD nMaxSge)
    {
        ...
        HRESULT hr_1 = m_pConnector->GetPrivateData(tmpBuf, &len);
        if (ND_SUCCESS != hr_1)
        {
            free(tmpBuf);
...
int __cdecl _tmain(int argc, TCHAR* argv[])
{
    ...
    int len_1 = sizeof(v4Server);
    WSAStringToAddress(argv[argc - 1], AF_INET, nullptr,
        reinterpret_cast<struct sockaddr*>(&v4Server), &len_1);
    ...
    HRESULT hr_2 = NdStartup();
    if (FAILED(hr_2))
    {
        LOG_FAILURE_HRESULT_AND_EXIT(hr_2, L"NdStartup failed with %08x", __LINE__);
    }
...
#pragma warning (suppress: 6001) // ignore unitialized memory warning for pBuf
    ...
    if (FAILED(hr_2))
    {
        LOG_FAILURE_HRESULT_AND_EXIT(hr_2, L"NdCleanup failed with %08x", __LINE__);
    }
